 ...
 *   viên hành chính), QLKAT (quản trị), PHKAT (phụ huynh) + số thứ tự,
 *   vd HVKAT00123.
...
const LOGIN_CODE_RE = /^(HVKAT|GVKAT|NVKAT|QLKAT|PHKAT)\d{1,10}$/;
...
export function codeEmail(code: string): string {
  ...
  if (c.startsWith("hvkat")) return `${c}@hocvien.katclass.vn`;
  if (c.startsWith("phkat")) return `${c}@phuhuynh.katclass.vn`;
  return `${c}@noibo.katclass.vn`;
}
