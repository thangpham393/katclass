...
- **Setup cần kiểm tra trước khi làm gì khác** (hỏi tôi nếu chưa chắc):
  1. Migrations đã dán vào Supabase SQL Editor theo thứ tự đến **`0011_library.sql`** (bảng textbooks + lessons.textbook_id). Nếu chưa dán 0011: mọi trang bài học (GV/HV/admin library) sẽ lỗi vì query embed `textbook:textbooks`.
