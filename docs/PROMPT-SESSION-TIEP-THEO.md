...
- **Đã xong**: đăng nhập + phân quyền 5 role; khu admin dữ liệu thật (dashboard, khóa học, lớp + lịch tuần, xếp học viên, phòng học, phân quyền); **khối giáo viên + vòng điểm danh vừa hoàn thành**:
  ...
  - **Đăng nhập DUY NHẤT bằng mã thành viên**: HVKAT (học viên), GVKAT (giáo viên), NVKAT (nhân viên), QLKAT (quản trị). Google/email đã gỡ khỏi UI (ô mã vẫn nhận email làm đường dự phòng); tài khoản lạ không có hồ sơ sẽ bị đăng xuất ngay (0009 khóa tự đăng ký — trigger chỉ nối theo email trùng, không tạo hồ sơ mới). Admin tạo học viên/GV/NV ở /admin/students, /admin/teachers → hệ thống TỰ cấp mã (trigger DB) + TỰ tạo tài khoản (API `/api/provision-account`, chỉ staff/admin gọi được, xác thực cookie phiên) với mật khẩu mặc định `kat12345` (hằng trong `src/lib/student-login.ts`); đăng nhập bằng mật khẩu mặc định bị đưa tới /account/password (trang này hiện luôn mã của chính mình). API `/api/student-auth` chỉ tra mã→email, KHÔNG tự tạo tài khoản. Cần env `SUPABASE_SERVICE_ROLE_KEY` (local + Vercel).
  - Migrations cần dán vào SQL Editor theo thứ tự: `0005_makeup_flow.sql` (vòng học bù), `0006_invite_codes.sql` (legacy claim_invite), `0007_student_codes.sql` (tiền tố HVKAT), `0008_team_codes.sql` (mã đội ngũ), `0009_lock_signup.sql` (khóa tự đăng ký). **KIỂM TRA đã chạy đủ chưa trước khi làm gì khác.**
